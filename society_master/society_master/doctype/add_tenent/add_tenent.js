...
/*cur_frm.cscript.house_no=function(doc,cdt,cdn)
{
    var l=doc.house_no;
    var w=doc.lane_no;
    var h=doc.house_type;
    frappe.call({
        method:'society_master.society_master.doctype.add_tenent.add_tenent.check_house',
        args:{l:l,w:w,h:h},
        callback:function(r)
        {
           if(r.message!=null)
           {
            cur_frm.set_value('lane_no','');
            cur_frm.set_value('wing_no','');
            cur_frm.set_value('house_type','');
            cur_frm.set_value('lane_name','');
            cur_frm.set_value('wing_name','');
            cur_frm.set_value('house_t','');
           }
        }
    })
}*/
